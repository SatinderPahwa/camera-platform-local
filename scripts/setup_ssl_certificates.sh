...
# Test if user can read certificates (using sg to test group access)
if sg ssl-certs -c "test -r $KEY_FILE" 2>/dev/null; then
    echo "   ✅ User can read private key via ssl-certs group"
else
    echo "   ⚠️  Warning: Group access test failed"
    echo "   This is normal if user hasn't logged out/in yet"
    echo "   Services will still work when started via managed_start.sh"
fi
echo ""
...
echo "   ✓ Set secure permissions on certificates (640 for privkey)"
echo "   ✓ Created Certbot renewal hook (preserves permissions)"
echo "   ✓ Updated .env with SSL configuration"
echo ""
echo "📌 Next Steps:"
echo "   1. Configure TURN server: sudo ./scripts/configure_turn_server.sh"
echo "   2. Restart services: ./scripts/managed_start.sh restart"
echo "   3. Test HTTPS access: https://$DOMAIN:5000"
echo ""
echo "💡 Note: Services will automatically use the ssl-certs group when started."
echo "   Interactive shell commands may require: newgrp ssl-certs (optional)"
echo ""
echo "🔍 Verify in logs (after restart):"
echo "   Gunicorn: tail -f $PROJECT_ROOT/logs/gunicorn_error.log | grep SSL"
echo "   Signaling: tail -f $PROJECT_ROOT/logs/livestreaming.log | grep -i ssl"
echo ""
