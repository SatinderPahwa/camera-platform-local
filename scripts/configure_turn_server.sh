...
if [ -z "$DOMAIN" ]; then
    echo "❌ Error: Could not detect domain from .env"
...
echo "📝 Creating turnserver configuration..."
...
echo "✅ Created turnserver configuration"
echo ""

# Set correct permissions
chmod 644 "$TURN_CONF"
echo "✅ Set permissions on turnserver.conf"
echo ""
...
if [ -f "/etc/default/coturn" ]; then
    if grep -q "^#TURNSERVER_ENABLED=1" /etc/default/coturn; then
        ...
        echo "✅ Enabled turnserver in /etc/default/coturn"
    elif grep -q "^TURNSERVER_ENABLED=1" /etc/default/coturn; then
        echo "✅ Turnserver already enabled in /etc/default/coturn"
    else
...
echo "============================================"
echo "📋 CoTURN Configuration Summary"
echo "============================================"
echo ""
...
if [ -f "$CERT_FILE" ] && [ -f "$KEY_FILE" ]; then
    ...
    if systemctl is-active --quiet coturn; then
        echo "✅ CoTURN service started successfully"
        systemctl enable coturn
        echo "✅ CoTURN service enabled (will start on boot)"
    else
    ...
    echo "⚠️  Skipping service start - SSL certificates not available yet"
    echo "💡 Run this script again after obtaining SSL certificates"
fi
...
echo "      - 3478 (UDP+TCP) → $LOCAL_IP:3478"
echo "      - 5349 (TCP) → $LOCAL_IP:5349"
echo "      - 49152-65535 (UDP) → $LOCAL_IP:49152-65535"
echo ""
...
echo ""
echo "📖 See DEPLOYMENT_GUIDE.md for firewall configuration"
echo ""
